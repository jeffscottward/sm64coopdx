...
- [x] Handle the `extract_assets.py` and ROM asset pipeline for web:
  ...
  > **Completed:** Verified all five aspects of the ROM asset pipeline for web compatibility. (1) `rom_assets_load()` uses standard C I/O (`fopen`/`fread`/`fseek`/`fclose`) at `rom_assets.c:173,67-68`, all fully emulated by Emscripten's VFS layer (MEMFS/IDBFS). The ROM file is written to `/save/baserom.us.z64` by `web_rom_loader.c` and `gRomFilename` is set to the same path by `rom_checker.cpp:90`. No adaptation needed. (2) `__attribute__((constructor))` is used by all `ROM_ASSET_LOAD_*` macros in `rom_assets.h` for auto-registration before `main()`. Emscripten fully supports GCC constructor attributes — verified by test with 3 constructor functions running before `main()`. (3) `tools/Makefile` hardcodes `CC := gcc` and `CXX := g++` (lines 6-7), meaning build tools (`n64graphics`, `textconv`, `skyconv`, etc.) always compile with the host toolchain regardless of `Makefile.web` overriding `CC="emcc"`. The main Makefile invokes `$(MAKE) -C tools/` which uses the tools' own CC. No adaptation needed. (4) Python scripts (`copy_extended_sounds.py`, `mario_anims_converter.py`, `demo_data_converter.py`) run via `$(PYTHON)` which resolves to host `python3` — they generate C source files at build time that are then compiled into WASM. No adaptation needed. (5) `ENDIAN_BITWIDTH` detection at `Makefile:1363` uses `$(CC)` (emcc for web) which correctly identifies WASM as little-endian 32-bit. Tests pass for both native and web-simulated builds.

- [x] Create a minimal HTML shell template `src/pc/web/shell.html` for the Emscripten output:
  ...
  > **Completed:** Created `shell.html` with three-screen flow: (1) ROM upload overlay with styled upload button and legal notice ("processed locally, never uploaded"), (2) loading overlay with progress bar driven by `Module.setStatus` that parses Emscripten's `(X/Y)` progress format, (3) error overlay with reload button triggered by `window.onerror`. The `Module` object configures: `canvas` with WebGL context-loss handler, `setStatus` with progress parsing, `onRuntimeInitialized` to show/resize canvas, `print`/`printErr` for console output. Includes viewport, mobile-web-app-capable, apple-mobile-web-app-capable, and theme-color meta tags. COOP/COEP header requirements documented in an HTML comment with explanation of SharedArrayBuffer dependency. ROM file types (`.z64`/`.n64`/`.v64`) referenced in the upload description. The Emscripten `{{{ SCRIPT }}}` placeholder is included for glue code injection. Added `--shell-file src/pc/web/shell.html` to `Makefile.web` WEB_LDFLAGS. All 30 validation tests pass; existing ROM startup tests pass without regression.
